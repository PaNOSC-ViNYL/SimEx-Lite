# Copyright Kartik Ayyer
# https://github.com/duaneloh/Dragonfly/blob/master/utils/py_src/detector.py
...
class Detector(object):
    ...
    def assemble_frame(self, data, zoomed=False, sym=False):
        ...
        if sym:
            self._init_sym()
            img = ma.masked_array(
                np.zeros(self._sym_shape, dtype="f8"), mask=1 - self._sym_mask
            )
            ...
            img.data[self._sym_bothgood] /= 2.0
            if zoomed:
                b = self._sym_zoom_bounds
                return img[b[0] : b[1], b[2] : b[3]]
        else:
            img = ma.masked_array(
                np.zeros(self.frame_shape, dtype="f8"), mask=1 - self.mask
            )
    ...
    def calc_from_coords(self):
        ...
        self.qz = self.ewald_rad * (self.detd / fac - 1.0)
        self.corr = self.detd / fac**3 * (1.0 - self.cx**2 / fac**2)
    ...
    def _parse_asciidet(self, mask_flag, keep_mask_1):
        ...
        self.qx, self.qy, self.qz, self.corr = tuple(
            [
                np.array(dframe[key])  # pylint: disable=C0103
    ...
    def _process_det(self, mask_flag, keep_mask_1):
        ...
        if self.qz.mean() > 0:
            self.cx = (
                self.qx * self.detd / (self.ewald_rad - self.qz)
            )  # pylint: disable=C0103
            self.cy = (
                self.qy * self.detd / (self.ewald_rad - self.qz)
            )  # pylint: disable=C0103
        else:
            self.cx = (
                self.qx * self.detd / (self.ewald_rad + self.qz)
    ...
    def _init_sym(self, force=False):
        if self._sym_shape is not None and not force:
            return
        self._sym_shape = (
            2 * int(np.ceil(np.abs(self.cx).max())) + 1,
        ...
        mask = self.unassembled_mask
        xsel = np.concatenate(
            (self._sym_x[mask.astype("bool")], self._sym_fx[mask.astype("bool")])
        )
        ysel = np.concatenate(
            (self._sym_y[mask.astype("bool")], self._sym_fy[mask.astype("bool")])
        )
